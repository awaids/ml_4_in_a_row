...
class FourInRowEnv(Env):
    ...
    def __init__(self, rows: int=6, cols: int=7) -> None:
        ...
        self.observation_space = spaces.Box(low=-1.0, high=1.0, shape=self.shape, dtype=np.int8)
        self.reset()
    ...
    def has_won(self) -> bool:
        ...
        def check_diagnols(state: ObsType) -> bool:
            ...
            diag_array1 = np.array([np.resize(np.diag(state, k=i), ncols,) for i in range(-ncols + 1 + Four, ncols + 1 - Four)])
            # Flip the array and determine the diagonals again
            h_flipped = np.fliplr(state)
            diag_array2 = np.array([np.resize(np.diag(h_flipped, k=i), ncols,) for i in range(-ncols + 1 + Four, ncols + 1 - Four)])
